...
class Solution {
    public int findInMountainArray(int target, MountainArray mountainArr) {
        ...
        int searchEle = binarySearch(0, peakElement, target, mountainArr);
        if(searchEle == -1) {
            searchEle = binarySearch(peakElement+1, mountainArr.length()-1, target, mountainArr);
        }

        return searchEle;
    }

    private int binarySearch(int start, int end, int target, MountainArray mountainArr){
